		...
		for(i=0; i<IMAGE_SIZE; i++)
		{
		    ...
		    for(j=0; j<IMAGE_SIZE; j++)
			{
				y = (double)j / IMAGE_SIZE;
               	...
               	y = y*1.5;
				// Perlin noise
				fire = 128 + 255*noise3(10*x, 3*y - time*2, time);
				if(fire<0) fire = 0;

				// Set red=grn=blu for grayscale image
				red = 3*fire*(1-sqrt(y));//3*temp*(1-pow(y, 2));
				grn = red - 255;//(3*temp-1)*(1-pow(y, 2));
				blu = 0;//3*temp-2;
				/*
				...
				blu = 0;
				*/
				smoke = 16 + 64*noise3(10*x, 3*y - time*0.5, time);
				
				red = smoke*pow(y, 2) + clamp(red);
				grn = smoke*pow(y, 2) + clamp(grn);
				blu = smoke*pow(y, 2) + clamp(blu);
